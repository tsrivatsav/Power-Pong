...
struct PongMode : Mode {
	...
	glm::vec2 court_radius = glm::vec2(8.0f, 6.0f);
	...
	uint32_t big_paddle = 0; //done
	uint32_t small_paddle = 0; //done
	uint32_t invisible_paddle = 0; //done

	uint32_t fast_ball = 0; //done
	uint32_t reverse_ball = 0; //done
	uint32_t tiny_ball = 0; //done

	uint32_t reverse_cntrl = 0; //done
	
	uint32_t spin = 0; //done
	float spin_factor = 1.01f;
	uint32_t spin_type = 0;
