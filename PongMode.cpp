...
bool PongMode::handle_event(SDL_Event const &evt, glm::uvec2 const &window_size) {
	...
	if (evt.type == SDL_MOUSEMOTION) {
		...
		//----- reverse controls power-up -----
		if (reverse_cntrl == 1) {
			left_paddle.y = 0.5f * court_radius.y - paddle_radius.y - 1.5f - (clip_to_court * glm::vec3(clip_mouse, 1.0f)).y;
		} else {
...
void PongMode::update(float elapsed) {
	...
	//----- power-up randomizer -----
	if (10000.0f * elapsed - float(int(10000.0f * elapsed)) == 0.0f) {
		// reset all power-ups
		big_paddle = 0; 
		small_paddle = 0; 
		invisible_paddle = 0; 
		...
		// select next power-up randomly
		int i = int(9.0f * mt() / float(mt.max()));
		switch (i) {
			case 1:
				big_paddle = 1;
				break;
	...
	{ //right player ai:
		...
		if (ai_offset_update < elapsed) {
			//update again in [0, 0.5) seconds:
			ai_offset_update = (mt() / float(mt.max())) * 0.5f;
	...
	//----- big/small paddle power-up -----

	if (big_paddle == 1) {
		paddle_radius.y = 2.5f;
	}
	else if (small_paddle == 1) {
		paddle_radius.y = 0.4f;
	}
	else {
		paddle_radius.y = 1.0f;
	}
	...
	//----- tiny ball power-up -----
	if (tiny_ball == 1) {
		ball_radius = glm::vec2(0.01f, 0.01f);
	} else {
...
void PongMode::draw(glm::uvec2 const &drawable_size) {
	...
	// ball colors
	const glm::u8vec4 red = HEX_TO_U8VEC4(0xed0505ff); //red
	const glm::u8vec4 light_blue = HEX_TO_U8VEC4(0x05d6edff); //light blue
	const glm::u8vec4 yellow = HEX_TO_U8VEC4(0xf5f105ff); //yellow
	const glm::u8vec4 green = HEX_TO_U8VEC4(0x05f50dff); //green
	const glm::u8vec4 purple = HEX_TO_U8VEC4(0x7105f5ff); //purple
	const glm::u8vec4 orange = HEX_TO_U8VEC4(0xf5a105ff); //orange
	const glm::u8vec4 brown = HEX_TO_U8VEC4(0x664301ff); //brown
	const glm::u8vec4 pink = HEX_TO_U8VEC4(0xf716e5ff); //pink
	const glm::u8vec4 white = HEX_TO_U8VEC4(0xffffffff); //white
	...
	//----- invisible paddle power-up -----
	if (invisible_paddle == 0) {
	...
	if (big_paddle == 1) {
		draw_rectangle(ball, ball_radius, red);
		draw_rectangle(ball + s, ball_radius, red);
	} else if (small_paddle == 1) {
		draw_rectangle(ball, ball_radius, light_blue);
		draw_rectangle(ball + s, ball_radius, light_blue);
	} else if (invisible_paddle == 1) {
		draw_rectangle(ball, ball_radius, yellow);
		draw_rectangle(ball + s, ball_radius, yellow);
	} else if (fast_ball == 1) {
		draw_rectangle(ball, ball_radius, green);
		draw_rectangle(ball + s, ball_radius, green);
	} else if (reverse_ball == 1) {
		draw_rectangle(ball, ball_radius, purple);
		draw_rectangle(ball + s, ball_radius, purple);
	} else if (tiny_ball == 1) {
		draw_rectangle(ball, ball_radius, orange);
		draw_rectangle(ball + s, ball_radius, orange);
	} else if (reverse_cntrl == 1) {
		draw_rectangle(ball, ball_radius, brown);
		draw_rectangle(ball + s, ball_radius, brown);
	} else if (spin == 1) {
		draw_rectangle(ball, ball_radius, pink);
		draw_rectangle(ball + s, ball_radius, pink);
	} else {
		draw_rectangle(ball, ball_radius, white);
		draw_rectangle(ball + s, ball_radius, white);
	}
